...
export class ShopMenusController {
  ...
  @Get('templates')
  @RequireShopPermission('menu_manage')
  async templates(@Req() req: RequestWithShop): Promise<{ success: true; data: TemplateRow[] }> {
    return { success: true, data: await this.menus.templates(req.shop!.shopId) };
  }

  @Post('clone-templates')
  @RequireShopPermission('menu_manage')
  async clone(@Req() req: RequestWithShop, @Body() dto: CloneTemplatesDto): Promise<{ success: true; data: { added: number } }> {
    const ids = Array.isArray(dto.templateIds) && dto.templateIds.length ? dto.templateIds.map(Number) : null;
    return { success: true, data: await this.menus.cloneTemplates(req.shop!.shopId, ids) };
  }

  @Post()
  @RequireShopPermission('menu_manage')
  @UseInterceptors(imageUpload)
  async create(@Req() req: RequestWithShop, @Body() dto: MenuDto, @UploadedFiles() files: UploadedImage): Promise<{ success: true; data: { id: number } }> {
  ...
  @RequireShopPermission('menu_manage')
  @UseInterceptors(imageUpload)
  async update(@Req() req: RequestWithShop, @Param('id', ParseIntPipe) id: number, @Body() dto: MenuDto, @UploadedFiles() files: UploadedImage): Promise<{ success: true }> {
  ...
  @Patch(':id/available')
  @RequireShopPermission('menu_manage')
  async toggle(@Req() req: RequestWithShop, @Param('id', ParseIntPipe) id: number, @Body() dto: ToggleDto): Promise<{ success: true }> {
